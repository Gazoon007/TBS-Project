...
namespace ConsolePro.Remote.LiteNetLib
{
	internal sealed class ReliableChannel
	{
		...
		{
			var validPacketSize = (_windowSize - 1) / BitsInByte + 1 + NetConstants.SequencedHeaderSize;
			...
			var startByte = NetConstants.SequencedHeaderSize;

			Monitor.Enter(_pendingPackets);
			for (var i = 0; i < _windowSize; i++)
			{
				var ackSequence = (ackWindowStart + i) % NetConstants.MaxSequence;
				...
				var currentByte = startByte + i / BitsInByte;
				var currentBit = i % BitsInByte;
				...
				var removed = _pendingPackets[ackSequence % _windowSize].GetAndClear();
				if (removed != null)
				{
		...
		public bool SendNextPacket()
		{
			...
			var startQueueIndex = _queueIndex;
			do
			{
				...
				if (currentPacket.Packet != null)
				{
					...
					{
						var packetHoldTime = (currentTime - currentPacket.TimeStamp.Value).TotalMilliseconds;
		...
		public void SendAcks()
		{
			...
			//Init packet
			var bytesCount = (_windowSize - 1) / BitsInByte + 1;
			var property = _ordered ? PacketProperty.AckReliableOrdered : PacketProperty.AckReliable;
			var acksPacket = _peer.GetPacketFromPool(property, bytesCount);

			//For quick access
			var data = acksPacket.RawData; //window start + acks size
			...
			//Put acks
			var startAckIndex = _remoteWindowStart % _windowSize;
			var currentAckIndex = startAckIndex;
			var currentBit = 0;
			var currentByte = NetConstants.SequencedHeaderSize;
			do
			{
		...
		public void ProcessPacket(NetPacket packet)
		{
			...
			var relate = NetUtils.RelativeSequenceNumber(packet.Sequence, _remoteWindowStart);
			var relateSeq = NetUtils.RelativeSequenceNumber(packet.Sequence, _remoteSequence);
