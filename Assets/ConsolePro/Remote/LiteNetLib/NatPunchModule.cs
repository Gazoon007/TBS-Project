...
namespace ConsolePro.Remote.LiteNetLib
{
	...
	public sealed class NatPunchModule
	{
		...
		public void SendNatIntroduceRequest(NetEndPoint masterServerEndPoint, string additionalInfo)
		{
			...
			var networkPort = _netBase.LocalEndPoint.Port;
		...
		private void HandleNatPunch(NetEndPoint senderEndPoint, NetDataReader dr)
		{
			...
			//Read info
			var additionalInfo = dr.GetString(MaxTokenLength);
			NetUtils.DebugWrite(ConsoleColor.Green, "[NAT] punch received from {0} - additional info: {1}",
				senderEndPoint, additionalInfo);
		...
		{
			// read intro
			var hostByte = dr.GetByte();
			var remoteInternal = dr.GetNetEndPoint();
			var remoteExternal = dr.GetNetEndPoint();
			var token = dr.GetString(MaxTokenLength);

			NetUtils.DebugWrite(ConsoleColor.Cyan,
				"[NAT] introduction received; we are designated " + (hostByte == HostByte ? "host" : "client"));
