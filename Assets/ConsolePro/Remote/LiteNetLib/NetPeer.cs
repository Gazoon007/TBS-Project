...
namespace ConsolePro.Remote.LiteNetLib
{
	...
	public sealed class NetPeer
	{
		...
		private void SendConnectRequest()
		{
			...
			var keyData = Encoding.UTF8.GetBytes(_peerListener.ConnectKey);
		...
		{
			//Prepare
			var property = SendOptionsToProperty(options);
			...
			if (length + headerSize > _mtu)
			{
				...
				var packetFullSize = _mtu - headerSize;
				var packetDataSize = packetFullSize - NetConstants.FragmentHeaderSize;

				var fullPacketsCount = length / packetDataSize;
				var lastPacketSize = length % packetDataSize;
				var totalPackets = fullPacketsCount + (lastPacketSize == 0 ? 0 : 1);
				...
				var dataOffset = headerSize + NetConstants.FragmentHeaderSize;
				for (ushort i = 0; i < fullPacketsCount; i++)
				{
					var p = _packetPool.Get(property, packetFullSize);
				...
				if (lastPacketSize > 0)
				{
					var p = _packetPool.Get(property, lastPacketSize + NetConstants.FragmentHeaderSize);
					p.FragmentId = _fragmentId;
			...
			//Else just send
			var packet = _packetPool.GetWithData(property, data, start, length);
			SendPacket(packet);
		}

		private void CreateAndSend(PacketProperty property, ushort sequence)
		{
			var packet = _packetPool.Get(property, 0);
		...
		internal void AddIncomingPacket(NetPacket p)
		{
			if (p.IsFragmented)
			{
				...
				//Increase total size
				var dataOffset = p.GetHeaderSize() + NetConstants.FragmentHeaderSize;
				...
				var resultingPacket = _packetPool.Get(p.Property, incomingFragments.TotalSize);

				var resultingPacketOffset = resultingPacket.GetHeaderSize();
				var firstFragmentSize = fragments[0].Size - dataOffset;
				for (var i = 0; i < incomingFragments.ReceivedCount; i++)
				{
					//Create resulting big packet
					var fragmentSize = fragments[i].Size - dataOffset;
					Buffer.BlockCopy(
						fragments[i].RawData,
						dataOffset,
		...
		internal void ProcessPacket(NetPacket packet)
		{
			...
			switch (packet.Property)
			{
				case PacketProperty.ConnectRequest:
					...
					var newId = BitConverter.ToInt64(packet.RawData, 1);
					if (newId > _connectId)
					{
						_connectId = newId;
					}

					NetUtils.DebugWrite("ConnectRequest LastId: {0}, NewId: {1}, EP: {2}", ConnectId, newId,
						_remoteEndPoint);
					SendConnectAccept();
					_packetPool.Recycle(packet);
					break;
				...
				case PacketProperty.Merged:
					...
					while (pos < packet.Size)
					{
						...
						pos += 2;
						var mergedPacket = _packetPool.GetAndRead(packet.RawData, pos, size);
				...
				case PacketProperty.Pong:
					...
					var rtt = (int) (DateTime.UtcNow - _pingTimeStart).TotalMilliseconds;
		...
		internal void SendRawData(NetPacket packet)
		{
			...
			{
				FastBitConverter.GetBytes(_mergeData.RawData, _mergePos + NetConstants.HeaderSize,
					(ushort) packet.Size);
		...
		private void SendQueuedPackets(int currentMaxSend)
		{
			...
			if (_mergePos > 0)
			{
				...
				else
				{
					...
					_peerListener.SendRaw(_mergeData.RawData, NetConstants.HeaderSize + 2, _mergePos - 2,
						_remoteEndPoint);
				}
		...
		internal void Update(int deltaTime)
		{
			...
			var maxSendPacketsCount = _peerListener.GetPacketsPerSecond(_currentFlowMode);
			int currentMaxSend;

			if (maxSendPacketsCount > 0)
			{
				var availableSendPacketsCount = maxSendPacketsCount - _sendedPacketsCount;
				...
				if (_mtuCheckTimer >= MtuCheckDelay)
				{
					...
					else
					{
						lock (_mtuMutex)
						{
							...
							if (_mtuIdx < NetConstants.PossibleMtu.Length - 1)
							{
								var newMtu = NetConstants.PossibleMtu[_mtuIdx + 1] - NetConstants.HeaderSize;
