...
namespace ConsolePro.Remote.LiteNetLib
{
	public sealed class NetManager
	{
		...
		internal bool SendRaw(byte[] message, int start, int length, NetEndPoint remoteEndPoint)
		{
			...
			var errorCode = 0;
			var result = _socket.SendTo(message, start, length, remoteEndPoint, ref errorCode) > 0;
		...
		private void UpdateLogic()
		{
			...
			lock (_peers)
			{
				var delta = _logicThread.SleepTime;
		...
		private void DataReceived(byte[] reusableBuffer, int count, NetEndPoint remoteEndPoint)
		{
			...
			//Try read packet
			var packet = _netPacketPool.GetAndRead(reusableBuffer, 0, count);
			if (packet == null)
			{
			...
			try
			{
				...
				{
					var protoId = BitConverter.ToInt32(packet.RawData, 1);
					...
					var peerKey = Encoding.UTF8.GetString(packet.RawData, 13, packet.Size - 13);
		...
		public bool SendUnconnectedMessage(byte[] message, int start, int length, NetEndPoint remoteEndPoint)
		{
			...
			var result = SendRawAndRecycle(packet, remoteEndPoint);
			return result;
		}
